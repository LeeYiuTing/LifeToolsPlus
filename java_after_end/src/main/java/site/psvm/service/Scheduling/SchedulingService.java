...
public class SchedulingService {
    ...
    public void procImage() {
        ...
        if (null != fileName) {
            // 添加线程信息日志
            logger.info("当前线程: {} - 开始处理图片任务: {}", Thread.currentThread().getName(), fileName);
            ...
            // 更新到es
            boolean update1 = resourceFileServiceImpl.update(wrapper);

            logger.info("图片生成缩略图: {}, 线程: {}", update, Thread.currentThread().getName());
        }
    }
